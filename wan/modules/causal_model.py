...
from wan.modules.dummyforcing import heterogeneous_memory_allocation, online_head_classification
...
class CausalWanSelfAttention(nn.Module):
    ...
    def forward(
        ...
        mid_context_len = self.args.local_context_length
        if cur_AR_step <= framecache_step or is_recache:
            ...
            if update_kv_cache:
                kv_cache['sink_k'] = k0[:, :HW]
                kv_cache['sink_v'] = v0[:, :HW]
                kv_cache['local_k'] = k0[:, -3*HW*mid_context_len:] if cur_AR_step > mid_context_len-1 else k0[:, HW:]
                kv_cache['local_v'] = v0[:, -3*HW*mid_context_len:] if cur_AR_step > mid_context_len-1 else v0[:, HW:]
        else:
            # suppose we have packed the first and last chunk into the sink_k and sink_v
            headgroup_first = kv_cache['headgroup_first']
            headgroup_mid = kv_cache['headgroup_mid']
            headgroup_last = kv_cache['headgroup_last']
            q1, k1, v1, q2, k2, v2 = extract_heads_triton(roped_query, roped_key, v, headgroup_mid, headgroup_first + headgroup_last)
            ...
            x[:, :, headgroup_mid, :] = x2
            if update_kv_cache:
                kv_cache['sink_k'][:, :, len(headgroup_first):] = k1[:, -HW:, len(headgroup_first):]
                kv_cache['sink_v'][:, :, len(headgroup_first):] = v1[:, -HW:, len(headgroup_first):]
                kv_cache['local_k'] = k2[:, -3*HW*mid_context_len:]
                kv_cache['local_v'] = v2[:, -3*HW*mid_context_len:]


        if cur_AR_step == framecache_step and update_kv_cache:
            self.frame_attn_score = online_head_classification(roped_query, k0, self.args.ar_start) # 24ms for sum layers
            kv_cache['frame_attn_score'] = self.frame_attn_score
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    def _forward_inference(
        ...
        if t[0,0]== self.args.denoise_start and AR_step == self.args.ar_start and not is_recache:
            # we calculate the three heads's assignment at the 1-st AR step and the t=0 denoising step
            heterogeneous_memory_allocation(kv_cache, num_dummy=self.args.num_dummy)
        # head
